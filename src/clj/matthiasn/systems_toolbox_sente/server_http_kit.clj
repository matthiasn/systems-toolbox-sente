(ns matthiasn.systems-toolbox-sente.server-http-kit
  ...
  (fn [put-fn]
    (let [undertow-cfg (merge {:host   (or env-host host)
          ...
          opts (merge {:user-id-fn user-id-fn
                      ...
                      sente-opts)
          ws (sente/make-channel-socket-server! (hk/get-sch-adapter) opts)
          ...
          server (http-kit/run-server wrapped-in-middleware undertow-cfg)
          host (:host undertow-cfg)
          port (:port undertow-cfg)
          shutdown-fn (fn []
                        (log/info "Stopping port" port "on interface" host)
                        (server))]
      (log/info "HTPP Kit listening on port" port "on interface" host)
      (when-let [ssl-port (:ssl-port undertow-cfg)]
        (log/info
          "HTPP Kit is listening on SSL-port" ssl-port "on interface" host))
